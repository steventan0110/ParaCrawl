...
output_dir=${ROOT}/datasets/sent_sim_score
...
for threshold in 0.0 0.6 0.7 0.8 0.9 0.95; do
  if [ ! -e ${output_file}-${threshold}.en ]; then
    echo "filter corpus with SBERT score " $threshold
    python filter_corpus_with_laser.py --lang ha --threshold ${threshold} --input $sent_score --output ${output_file}
  fi
done
...
datasets=${ROOT}/datasets/sent_sim_score
...
for threshold in 0.0 0.6 0.7 0.8 0.9 0.95; do
  fairseq-preprocess \
    ...
    --destdir $ROOT/data-bin/ha-en-sent-sim-${threshold} \
    --workers 8
done
